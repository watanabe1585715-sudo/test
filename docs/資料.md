...
実装の概要図・代表的なフロー・HTTP API 一覧です。  
画面一覧とスクリーンショット格納先は [画面一覧.md](画面一覧.md) / [screenshots/README.md](screenshots/README.md) を参照してください。

---

## 各システムの概要と使用言語・技術スタック

要件（`test`）どおりフロントは用途ごとに **別プロジェクト**、バックエンドとバッチは **同一 Go モジュール**（`recruitment`）に集約しています。

### 求人サイト（公開）— `frontend-react`

| 項目 | 内容 |
|------|------|
| 概要 | 求職者向け。トップ兼一覧、案件詳細モーダル、応募モーダル。**会員登録・ログイン・マイページ（プロフィール編集）**は JWT（`job_seeker`）で `/job-seeker/*` を利用。 |
| 言語 | **TypeScript**（`.tsx`） |
| UI | **React 18** |
| ビルド／開発 | **Vite 5** |
| 既定ポート | `5173` |
...
| 項目 | 内容 |
|------|------|
| 概要 | 広告主（顧客）側オペレータ向け。ログイン、案件の一覧・検索・作成／更新／削除モーダル、応募者一覧。自顧客に紐づく案件のみ。 |
...
| 言語 | **TypeScript**（`.tsx`） |
| UI | **React 18** |
| フレームワーク | **Next.js 14**（App Router）。画面は `"use client"` を中心としたクライアントコンポーネント。 |
| 既定ポート | `3000` |
...
| 項目 | 内容 |
|------|------|
| 概要 | 三フロント共通の **REST（JSON）**。`/public`（匿名）、`/job-seeker`（求職者 JWT）、`/admin/jobs`（案件管理 JWT）、`/admin/customers`（顧客管理 JWT）。起動時に **SQL マイグレーション** を実行。 |
| 言語 | **Go** 1.22 想定 |
...
| エントリ | `cmd/batch` |
| 実行 | CLI。`docker compose --profile tools run --rm batch` または `go run ./cmd/batch`。タイムゾーン **Asia/Tokyo**（`BATCH_DATE` で基準日を上書き可）。 |
...
接続ホスト・ポート・ユーザー等は **[データベース接続.md](データベース接続.md)**（A5:SQL Mk-2 の手順付き）を参照してください。
...
優先順位（実装）: 同一顧客内で `publish_start` 昇順 → `created_at` 昇順 → `id` 昇順。契約 tier 1=10件 / 2=100件 / 3=無制限。
...
ベース URL 例: `http://localhost:8080`。認証が必要なエンドポイントは `Authorization: Bearer <JWT>`。

**ハンドラ列**は実装ファイル [handlers.go](../internal/interfaces/http/handlers.go) 内の **関数名**（ルート登録は [router.go](../internal/interfaces/http/router.go) のコメント参照）。
...
公開求人・応募は **`customers.approval_status = approved`** の顧客の案件のみ（未承認顧客は掲載されません）。
...
`contact` に `@` を含む場合、応募保存と同一トランザクションで **サンキューメール** が `email_outbox` に `pending` で積まれます（送信は `cmd/mailworker` と SMTP 設定が必要）。
...
| 顧客管理 | `customer_admin` | `cid` は 0。顧客スコープは API 側で全件可 |
...
以下は今回要望の追加機能を **実装済み API** として整理したものです。  
